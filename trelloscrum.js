...
function ListCard(el, identifier) {
	...
	this.refresh = function () {
		...
		if ($card.parent()[0]) {
			...
			$badge.attr({
				title: 'This card has ' + that.points + (consumed ? ' consumed' : '') + ' storypoint' + (that.points === 1 ? '.' : 's.')
			});
		}
	};
...
function List(el) {
		...
		$c.each(function () {
			...
			if (!that.listCard) {
				...
				$(that)
					...
					if (!busy && ($(e.target)
						.hasClass('list-card-title') || e.target === that)) {
						clearTimeout(to22);
	...
	$list.on('DOMNodeInserted', function (e) {
		if ($(e.target)
			.hasClass('list-card') && !e.target.listCard) {
			clearTimeout(to2);
...
function showExcelExport() {
	...
	$.getJSON($('form')
		...
		s = '<table id="export" border=1>' + '<tr><th>Points</th><th>Story</th><th>Description</th></tr>';
...
$(function () {
	...
	// watch filtering
	$(document)
		.on('mouseup', '.js-filter-toggle', function (e) {
		setTimeout(function () {
			filtered = $('.js-filter-cards')
				.hasClass('is-on');
			calcPoints();
		});
	});
	...
	$(document)
		.on('DOMNodeInserted', '.card-detail-title .edit-controls', function () {
		if ($(this)
			.find('.picker')
			.length) {
			return;
		...
		function registerClickEvent(element) {
			element.click(function () {
				...
				$(".card-detail-title .edit .js-save-edit")
					.click();
		...
		for (p in pointSeq) {
			if (pointSeq.hasOwnProperty(p)) {
				...
				$pickerPoint = $('<span class="point-value point-value-' + point + '">')
					.text(point);
	...
	$('body')
		.on('DOMSubtreeModified DOMNodeInserted', function (e) {
		if ($(e.target)
			.hasClass('list')) {
			readList($(e.target));
			computeTotal();
